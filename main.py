...
try:
    ...
    from gymnasium.envs.registration import register as gym_register, registry as gym_registry
...
for env_id, env_kwargs in DISSC_V3_FORAGING_SPECS:
    for registered_id in (env_id, f"lbforaging:{env_id}"):
        if registered_id not in gym_registry:
            register(
                id=registered_id,
                entry_point="lbforaging.foraging:ForagingEnv",
                kwargs=deepcopy(env_kwargs),
            )
